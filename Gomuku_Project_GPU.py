...
try:
    ...
    while running:
        ...
        lines = [
            ...
            "  /opt/anaconda3/bin/python3 Gomuku_Project_GPU.py",
            "",
...
pygame.display.set_caption("Omok 13x13")
...
# --- GPU 가속 배치 벡터화 몬테카를로 롤아웃 (MCTS) ---
def get_action_mcts_vectorized(agent, board_state, current_turn, open3_rule, num_rollouts=30, top_k=5):
    ...
    batch_size = num_candidates * num_rollouts
    ...
    for i, cand in enumerate(candidates):
        for j in range(num_rollouts):
            ...
            cr, cc = cand
            boards_batch[idx][cr][cc] = current_turn
    ...
    while sim_steps < max_sim_steps and np.any(active_mask):
        ...
        for idx in active_indices:
            b_arr = boards_batch[idx]
            op = 3 - turns_batch[idx]
            ch0 = (b_arr == turns_batch[idx]).astype(np.float32)
            ch1 = (b_arr == op).astype(np.float32)
            ch2 = (b_arr == 0).astype(np.float32)
            state_list.append(np.stack([ch0, ch1, ch2], axis=0))
            
        # GPU 전송 (Batch Tensor)
        batch_states_tensor = torch.tensor(np.array(state_list)).to(agent.device)
        
        agent.model.eval()
        with torch.no_grad():
            batch_logits = agent.model(batch_states_tensor).cpu().numpy()
            
        for step_idx, idx in enumerate(active_indices):
            b_arr = boards_batch[idx]
            sim_turn = turns_batch[idx]
            
            valid_idx = np.where(b_arr.flatten() == 0)[0]
            if len(valid_idx) == 0:
                active_mask[idx] = False
                continue
                
            logits = batch_logits[step_idx]
            mask_arr = np.zeros(169, dtype=np.float32)
            mask_arr[valid_idx] = 1.0
            
            # Epsilon-greedy (탐색용)
            if np.random.rand() < 0.15:
                chosen_idx = np.random.choice(valid_idx)
            else:
                logits[mask_arr == 0] = -1e9
                exp_logits = np.exp(logits - np.max(logits))
                probs = exp_logits / np.sum(exp_logits)
                chosen_idx = np.random.choice(169, p=probs)
                
            sr, sc = chosen_idx // agent.board_size, chosen_idx % agent.board_size
            boards_batch[idx][sr][sc] = sim_turn
            
            if sim_check_win(boards_batch[idx], sr, sc, sim_turn):
                winners_batch[idx] = sim_turn
                active_mask[idx] = False
            else:
    ...
    print("\n--- AI Move Candidate Win Rates (GPU MCTS) ---")
    for i, cand in enumerate(candidates):
        start = i * num_rollouts
        end = start + num_rollouts
        cand_winners = winners_batch[start:end]
        
        wins = np.sum(cand_winners == current_turn)
        losses = np.sum(cand_winners == opponent)
        draws = np.sum(cand_winners == 0)
        score = wins + 0.2 * draws
        
        win_rate = (wins / num_rollouts) * 100
        print(f"Candidate {i+1}: Move {cand} | Win: {win_rate:.1f}% (W: {wins}, L: {losses}, D: {draws})")
    ...
    def __init__(self, board_size=13, lr=0.0005):
        ...
        if os.path.exists(self.saved_model_path):
            try:
                ...
                print(f"Loaded saved weights successfully on: {self.device}")
...
while running:
    ...
    # AI Turn (Vectorized MCTS on GPU)
    if state == "GAME" and mode == "AI" and turn == 2 and not winner:
        pygame.time.delay(100)
        if difficulty == "Hard":
            draw_board()
            draw_text("AI is reading ahead (GPU Accelerated)...", SCREEN_SIZE // 2, (220, 0, 0))
            pygame.display.flip()
            move = get_action_mcts_vectorized(rl_agent, board, turn, rule_open3, num_rollouts=50, top_k=5)
        else:
    ...
    if state == "MENU_MODE":
        screen.fill(WHITE)
        draw_text("Omok 13x13 AI System", 100, (0, 0, 150))
        draw_text("1. Local 1 vs 1 Game", 220)
        draw_text("2. Player vs AI (GPU RL)", 280)
        ...
        screen.fill(WHITE)
        draw_text("Select AI Difficulty Level (GPU)", 180, (0, 0, 150))
        draw_text("1. Easy", 260)
        draw_text("2. Medium", 320)
        draw_text("3. Hard - MCTS 50", 380)
        ...
        if winner: 
            ...
            if mode == "AI":
                draw_text(f"Difficulty: {difficulty} (GPU)", 57, (120, 120, 120))
        ...
        draw_train_text("AI Self-Play Training (GPU)...", 200, (220, 0, 0))
